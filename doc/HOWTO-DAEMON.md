...
The `compression` can take on various values depending on the Kafka layer.  For the current
librdkafka layer the value `lz4` is recommended (and it is recommended that the compression property
of the broker is set to `producer` to allow compressed data to pass through without processing).

the values `gzip`, `snappy`, or `none` and if set to something other than
