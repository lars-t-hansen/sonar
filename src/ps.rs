    ...
    match collect_sample_data(system, opts) {
        Ok(Some(sample_data)) => {
            if opts.new_json {
                ...
                if elements.len() == 0 && opts.always_print_something {
...
fn usertable_from_processes<T>(procs: &HashMap<T,procfs::Process>) -> UserTable {
...
fn collect_sample_data(
    ...
    let (processes, _cpu_total_secs, per_cpu_secs) = {
        let memtotal_kib = procfs::get_memtotal_kib(system.get_procfs())?;
...
fn add_gpu_info(
    ...
    if let Some(gpu) = system.get_gpus().probe() {
        ...
        match gpu.get_process_utilization(&usertable_from_processes(&processes)) {
            Ok(mut gpu_utilization) => {
                // Tweak gpu_utilization: If any entry used more than 1 GPU we split the entry into
                // one per GPU, with cumulative absolute data divided by the number of GPUs.
                // (Percentage data remain unchanged.)
                ...
                for proc in &mut gpu_utilization {
                    ...
                    if l > 1 {
                        ...
                        for d in devices.drain(1..) {
                            ...
                            c.devices[0] = d;
                            c.mem_size_kib = proc.mem_size_kib / l;
                            additional.push(c)
                        }
                    }
                }
