use crate::gpuapi;
use crate::output;
use crate::ps::{self,GpuStatus,ProcInfo,PsOptions,SampleData};
use crate::systemapi;
...
pub fn format_oldfmt(c: &SampleData, system: &dyn systemapi::SystemAPI, opts: &PsOptions) -> output::Array {
...
fn format_oldfmt_sample(proc_info: &ProcInfo, system: &dyn systemapi::SystemAPI) -> output::Object {
    ...
    if !proc_info.is_slurm {
        fields.push_u("epoch", system.get_boot_time() - ps::EPOCH_TIME_BASE);
    }
    ...
    if proc_info.gpus.is_empty() {
        ...
        fields.push_s(
            ...
            proc_info.gpus
                .iter()
                .map(|(device,_)| device.index.to_string())
                .collect::<Vec<String>>()
                .join(","),
        );
    }
