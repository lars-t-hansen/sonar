            ...
            match *field {
                ...
                "TimelimitRaw" => {
                    if fieldvals[i] == "UNLIMITED" {
                        output_line.push_i(SLURM_JOB_TIMELIMIT, -1); // "Infinite"
                    } else if fieldvals[i] != "Partition_limit" {
                        push_uint_full(&mut output_line, SLURM_JOB_TIMELIMIT, &fieldvals[i], 60, 1, false);
                    }
                }
                "NodeList" => {
                    if fieldvals[i] != "" {
                ...
                "Priority" => {
                    push_uint_full(&mut output_line, SLURM_JOB_PRIORITY, &fieldvals[i], 1, 1, false);
                }
                "JobName" => {
                ...
                "AveDiskRead" | "AveDiskWrite" | "AveRSS" | "AveVMSize" | "MaxRSS" | "MaxVMSize" => {
                    // FIXME
                    // SACCT_DATA_AVE_DISK_READ
                    // SACCT_DATA_AVE_DISK_WRITE
                    // SACCT_DATA_AVE_RSS
                    // SACCT_DATA_AVE_VMSIZE
                    // SACCT_DATA_MAX_RSS
                    // SACCT_DATA_MAX_VMSIZE
                ...
                "AveCPU" | "MinCPU" | "UserCPU" | "SystemCPU" => {
                    ...
                    // SACCT_DATA_MIN_CPU
                    // SACCT_DATA_USER_CPU
                    // SACCT_DATA_SYSTEM_CPU
                    push_duration(&mut sacct, *field, &fieldvals[i]);
                }
                "ElapsedRaw" => {
                    // FIXME
                    // SACCT_DATA_ELAPSED_RAW
                    push_uint(&mut sacct, *field, &fieldvals[i]);
                }
                "AllocTRES" => {
                    // FIXME
                    // SACCT_DATA_ALLOC_TRES
                    push_string(&mut sacct, *field, &fieldvals[i]);
                }
                //-implicit-use

                // push_s(SLURM_JOB_GRESDETAIL, _)
                // push_s(SLURM_JOB_MIN_CPUSPER_NODE, _)
