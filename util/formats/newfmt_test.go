...
//
// Strings found between //+implicit-use and //-implicit-use are marked as used, this is a hack for
// the slurm stuff which is table-driven.

func TestFieldNames(t *testing.T) {
	push_re := regexp.MustCompile(`\.push_.\([^"]*"([a-zA-Z0-9_-]*)`)
	string_re := regexp.MustCompile(`"([a-zA-Z0-9_-]*)"`)
	...
	for _, fn := range []string{
		...
		for b.Scan() {
			...
			if implicitUse {
				if r := string_re.FindStringSubmatch(l); r != nil {
					if r[1] != "" {
						usedStrings[r[1]] = fmt.Sprint(fn, ":", ln)
					}
				}
